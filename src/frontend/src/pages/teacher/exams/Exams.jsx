...
const PageHeader = () => (
  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
    <div>
      ...
      <p className="mt-1 text-muted-foreground">Create and manage your exams.</p>
    </div>
  </div>
);
                ...
                <div className="flex flex-col h-full justify-between space-y-2">
                  <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">
                      {formatDate(upcomingExams[0].start_date)}, {" "}
                      {formatDateTime(upcomingExams[0].start_date)}
                    </p>
                  </div>
                </div>
              ) : (
          ...
          <Card className="flex flex-col h-full">
            ...
            <CardContent className="flex-1">
              <div className="space-y-1">
                ...
                <p className="text-sm text-muted-foreground">Start building an assessment</p>
              </div>
            </CardContent>
            <CardFooter className="pt-0">
...
const ExamsForSelectedDate = ({
  ...
  <Card className="col-span-full lg:col-span-1 border">
    <CardHeader className="pb-2">
      ...
      <CardDescription>
        {listExams.length === 1 ? "1 exam scheduled" : `${listExams.length} exams scheduled`}
      </CardDescription>
    </CardHeader>
    <CardContent>
          ...
          <ul className="space-y-3">
            {listExams.map((exam) => (
              <li
                key={
                  exam.id ||
                  `exam-${exam.title}-${new Date(exam.start_date).getTime()}`
                }
                className="relative"
              >
                <button
            ...
            {pastExams.length > 0 ? (
              <ul className="space-y-3">
                {pastExams
                  ...
                  .map((exam) => (
                    <li
                      key={exam.id || `past-${exam.title}-${exam.end_date}`}
                    >
                      <button
                        onClick={() => handleExamClick(exam)}
            ...
            {ongoingExams.length > 0 ? (
              <ul className="space-y-3">
                {ongoingExams.map((exam) => (
                  <li key={exam.id || `ongoing-${exam.title}-${exam.end_date}`}>
                    <button
                      onClick={() => handleExamClick(exam)}
                      className="w-full text-left p-3 rounded-lg border bg-card hover:bg-muted transition-colors"
                    >
        ...
        <ScrollArea className="h-[250px] pr-4">
          {nextExams.length > 0 ? (
            <ul className="space-y-3">
              {nextExams.map((exam) => (
                <li
                  key={exam.id || `upcoming-${exam.title}-${exam.start_date}`}
                  className="relative"
                >
                  <button
                    onClick={() => handleExamClick(exam)}
...
export default function TeacherExams() {
  ...
  const { getTimeStatus, getStatusConfig } = useExamStatus();
  
  const getStatusBadge = (exam, size = "sm") => {
    const status = getTimeStatus(exam.start_date, exam.end_date);
    const config = getStatusConfig(status);
    return <StatusBadge config={config} size={size} />;
  };
  ...
  const upcomingExams = exams.filter((e) => new Date(e.start_date) > now);
  ...
  const listExams = exams
    ...
    .filter((e) => e.title.toLowerCase().includes(searchQuery.toLowerCase().trim()));
